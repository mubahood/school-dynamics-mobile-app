...
class EmployeeModel {
  ...
  static Future<List<EmployeeModel>> getLocalData({String where = "1"}) async {
    List<EmployeeModel> data = [];
    if (!(await EmployeeModel.initTable())) {
  ...
  static Future<List<EmployeeModel>> get_items({String where = '1'}) async {
    List<EmployeeModel> data = await getLocalData(where: where);
    if (data.isEmpty) {
      await EmployeeModel.getOnlineItems();
      data = await getLocalData(where: where);
    } else {
      EmployeeModel.getOnlineItems();
    }
    return data;
  }

  static Future<List<EmployeeModel>> getOnlineItems() async {
    List<EmployeeModel> data = [];
    ...
    if (resp.data.runtimeType.toString().contains('List')) {
      ...
      await db.transaction((txn) async {
        ...
        for (var x in resp.data) {
          EmployeeModel sub = EmployeeModel.fromJson(x);
          try {
  ...
  static deleteAll() async {
    ...
    await db.delete(EmployeeModel.tableName);
  }
